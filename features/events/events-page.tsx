...
import { CalendarDays, Clock3, MapPin, Music3, Users2 } from "lucide-react";
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="space-y-3">
        <Badge variant="brandSoft" className="w-fit">Calendario juvenil</Badge>
        <h1 className="text-3xl font-black text-brand-foreground sm:text-4xl">Eventos</h1>
        <p className="max-w-2xl text-brand-muted">
          Encuentros creados para adorar, conectar y servir. Aqui puedes ver lo que viene para la comunidad.
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {events.map((event) => (
          <Card key={event.title} variant="surfaceSoft" className="h-full">
            <CardHeader>
              <Badge variant="brandSecondary" className="w-fit">Proximo</Badge>
              <CardTitle className="text-2xl text-brand-foreground">{event.title}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 pb-6 text-brand-muted">
              <p>{event.description}</p>
              <div className="space-y-2 text-sm text-brand-foreground">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          { icon: Music3, title: "Adoracion", text: "Noche centrada en musica y palabra." },
          { icon: Users2, title: "Conexion", text: "Actividades para crear comunidad real." },
          { icon: Clock3, title: "Ritmo constante", text: "Agenda pensada para que siempre haya un paso siguiente." },
        ].map((item) => {
          ...
          return (
            <Card key={item.title} variant="elevated">
              <CardHeader>
                ...
                <CardTitle className="text-xl text-brand-foreground">{item.title}</CardTitle>
              </CardHeader>
              <CardContent className="pb-6 text-brand-muted">{item.text}</CardContent>
            </Card>
          );
        })}
      </div>
      ...
      <Card variant="gradient">
        <CardContent className="flex flex-col gap-4 py-8 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            <h2 className="text-2xl font-black text-brand-foreground">Quieres participar en el proximo encuentro?</h2>
            <p className="text-brand-foreground/85">Sigue nuestras publicaciones o unete a un grupo de conexion para recibir novedades.</p>
          </div>
